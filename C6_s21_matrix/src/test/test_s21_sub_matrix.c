...
START_TEST(s21_sub_matrix_1) {
  ...
  int _return = s21_sub_matrix(&A, &B, &C);
...
START_TEST(s21_sub_matrix_5) {
  ...
  int _return = s21_sub_matrix(&A, NULL, &B);
  ck_assert_int_eq(_return, INCORRECT_MATRIX);
  s21_remove_matrix(&A);
}
END_TEST
...
START_TEST(s21_sub_matrix_6) {
  ...
  int _return = s21_sub_matrix(&A, &B, NULL);
  ck_assert_int_eq(_return, INCORRECT_MATRIX);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
}
...
Suite *suite_sub() {
  ...
  tcase_add_test(tc, s21_sub_matrix_1);  // ок
  tcase_add_test(tc, s21_sub_matrix_2);  // разные размеры
  tcase_add_test(tc, s21_sub_matrix_3);  // одна матрица неправильная
  tcase_add_test(tc, s21_sub_matrix_4);  // одна матрица с NULL-матрицей
  tcase_add_test(tc, s21_sub_matrix_5);  // одна матрица чистый NULL
  tcase_add_test(tc, s21_sub_matrix_6);  // результирующая матрица NULL
